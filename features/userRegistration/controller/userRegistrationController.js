...
const registerUser = async (req, res) => {
  const { accountType, userName, userEmail, userPhoneNumber, userPassword } =
    req.body;
  ...
  if (
    ...
    res.status(400).json({
      ...
      successfull: false,
      message:
        "accountType,userName,userEmail,userPhoneNumber and userPassword fields are required to continue.",
    });
  ...
  try {
    ...
    const token = await jsonWebToken.sign(user._doc, process.env.SECRET_KEY, {
      expiresIn: "30d",
    });
