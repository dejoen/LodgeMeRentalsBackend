    ...
    .use(async (socket, next) => {
        ...
        next(
          new Error(
            JSON.stringify({
              ...
              message:
                "Unathourize access. Token is needed to access this feature",
            }),
          ),
        );
      ...
      tokenValidator.verify(
        ...
        async (err, decoded) => {
          ...
          const isUserRegistered = await UsersModel.findOne({
    ...
    .on("connection", async (socket) => {
      ...
      if (user) {
        await user.updateOne({
          userSocketConnectionId: socket.id,
          isOnline: true,
        });
      }

      const houses = await UserPublishedHouses.find({
        publisher: socket.user._id,
      });

      if (socket.user.accountType === "agent") {
        const connectedUser = await UsersModel.findOne(
          { _id: socket.user._id },
      ...
      socket.on("disconnect", async () => {
        ...
        const disconnectedUser = await UsersModel.findOne({
          _id: socket.user._id,
        });
